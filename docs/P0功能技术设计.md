...
**命令**: `mail-cli reply <email-id> [options]`
...
**命令**: `mail-cli forward <email-id> [options]`
...
mail-cli forward 123 --to user@example.com --body "FYI"
```
...
**命令集**:
- `mail-cli draft save [options]`: 保存草稿
- `mail-cli draft list`: 列出所有草稿
- `mail-cli draft edit <draft-id>`: 编辑草稿
- `mail-cli draft delete <draft-id>`: 删除草稿
- `mail-cli draft send <draft-id>`: 发送草稿

**选项**:
```bash
mail-cli draft save --to user@example.com --subject "Test" --body "Content"
mail-cli draft list
mail-cli draft edit 5
mail-cli draft send 5
```
...
**命令集**:
- `mail-cli delete <email-id>`: 删除邮件（移到垃圾箱）
- `mail-cli delete <email-id> --permanent`: 永久删除
- `mail-cli trash list`: 查看垃圾箱
- `mail-cli trash empty`: 清空垃圾箱
- `mail-cli trash restore <email-id>`: 恢复邮件
...
**命令集**:
- `mail-cli spam mark <email-id>`: 标记为垃圾邮件
- `mail-cli spam unmark <email-id>`: 取消垃圾邮件标记
- `mail-cli spam list`: 查看垃圾邮件
- `mail-cli spam blacklist add <email>`: 添加到黑名单
- `mail-cli spam blacklist remove <email>`: 从黑名单移除
- `mail-cli spam whitelist add <email>`: 添加到白名单
- `mail-cli spam filter`: 手动运行垃圾邮件过滤
...
- `mail-cli signature list`: 列出所有签名
- `mail-cli signature edit <id>`: 编辑签名
- `mail-cli signature delete <id>`: 删除签名
- `mail-cli signature set-default <id>`: 设置默认签名

**示例**:
```bash
mail-cli signature create --name "Work" --text "Best regards, John"
mail-cli signature list
mail-cli signature set-default 1
```
