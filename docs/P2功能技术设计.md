...
```bash
# 创建模板
mail-cli template create
mail-cli template create --name "Meeting Request" --subject "Meeting: {{topic}}" --body "Hi {{name}}, ..."

# 列出模板
mail-cli template list
mail-cli template list --category work

# 查看模板详情
mail-cli template show <id|name>

# 编辑模板
mail-cli template edit <id|name>

# 删除模板
mail-cli template delete <id|name>

# 使用模板发送邮件
mail-cli send --template <id|name> --var name=John --var topic="Q1 Review"
```
...
mail-cli notify config --desktop on --sound off

# 免打扰模式
mail-cli notify dnd on
mail-cli notify dnd on --until "2026-02-08 18:00"
mail-cli notify dnd off

# 通知过滤规则
mail-cli notify filter add --from important@company.com
mail-cli notify filter add --subject "urgent"
mail-cli notify filter list
mail-cli notify filter remove <id>

# 查看通知历史
mail-cli notify history
mail-cli notify history --limit 20

# 后台监听模式
mail-cli notify watch
mail-cli notify watch --interval 60  # 每60秒检查一次
```
...
```bash
# 启动后台监听
mail-cli notify watch --daemon

# 停止后台监听
mail-cli notify stop
```
...
```bash
# 导出单封邮件为EML
mail-cli export eml <email-id> --output ./email.eml

# 导出文件夹为MBOX
mail-cli export mbox --folder INBOX --output ./inbox.mbox

# 导出日期范围
mail-cli export mbox --from 2026-01-01 --to 2026-01-31 --output ./january.mbox

# 导出所有邮件
mail-cli export all --output ./backup --format mbox

# 仅导出附件
mail-cli export attachments <email-id> --output ./attachments/

# 完整备份（包括数据库）
mail-cli backup --output ./full-backup.zip
```

#### 导入命令
```bash
# 从EML导入
mail-cli import eml ./email.eml --folder INBOX

# 从MBOX导入
mail-cli import mbox ./inbox.mbox --folder INBOX

# 批量导入目录中的EML文件
mail-cli import dir ./emails/ --folder INBOX

# 恢复完整备份
mail-cli restore ./full-backup.zip

# 验证导入文件
mail-cli import validate ./inbox.mbox
```
