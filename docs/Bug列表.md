# Bug 列表

## 文档信息
- **项目**: 邮箱客户端 P0 功能
- **版本**: v1.0.0
- **测试日期**: 2026-02-08
- **测试人员**: QA Team

---

## Bug 统计

| 严重程度 | 数量 | 状态 |
|---------|------|------|
| 🔴 阻塞性 (Blocker) | 0 | - |
| 🟠 严重 (Critical) | 0 | - |
| 🟡 一般 (Major) | 0 | - |
| 🟢 轻微 (Minor) | 0 | - |
| **总计** | **2** | **已修复** |

---

## 已修复问题

### Bug #1: 缺少依赖包 cli-table3

#### 基本信息
- **Bug ID**: BUG-001
- **严重程度**: 🔴 阻塞性 (Blocker)
- **优先级**: P0 (最高)
- **状态**: ✅ 已修复
- **发现日期**: 2026-02-08
- **修复日期**: 2026-02-08
- **发现人**: QA Team
- **修复人**: Development Team
- **影响版本**: v1.0.0
- **修复版本**: v1.0.1

#### 问题描述
在运行 `draft list` 和 `trash list` 命令时，程序抛出模块未找到错误，导致这两个命令完全无法使用。

#### 错误信息
```
Error: Cannot find module 'cli-table3'
Require stack:
- D:\code\mail\src\cli\commands\draft.js
- D:\code\mail\src\cli\index.js
- D:\code\mail\src\index.js
```

#### 复现步骤
1. 安装邮箱客户端
2. 运行命令: `mail-cli draft list`
3. 或运行命令: `mail-cli trash list`
4. 观察到错误

#### 影响范围
- ❌ `mail-cli draft list` - 无法运行
- ❌ `mail-cli trash list` - 无法运行
- ✅ 其他命令 - 不受影响

#### 受影响文件
- `src/cli/commands/draft.js:4`
- `src/cli/commands/trash.js:4`
- `package.json` (缺少依赖声明)

#### 根本原因
在开发过程中使用了 `cli-table3` 包来格式化表格输出，但忘记在 `package.json` 中声明该依赖。

#### 修复方案

在 `package.json` 中添加了缺失的依赖：
```json
{
  "dependencies": {
    "cli-table3": "^0.6.3"
  }
}
```

#### 修复验证
- ✅ `mail-cli draft list` - 正常运行
- ✅ `mail-cli trash list` - 正常运行
- ✅ 表格显示正常

---

### Bug #2: delete.js引用不存在的account模块

#### 基本信息
- **Bug ID**: BUG-002
- **严重程度**: 🔴 阻塞性 (Blocker)
- **优先级**: P0 (最高)
- **状态**: ✅ 已修复
- **发现日期**: 2026-02-08
- **修复日期**: 2026-02-08
- **发现人**: User
- **修复人**: Development Team
- **影响版本**: v1.0.0
- **修复版本**: v1.0.1

#### 问题描述
在运行邮件删除命令时，程序抛出模块未找到错误。`delete.js` 文件中引用了不存在的 `account` 模块。

#### 错误信息
```
Error: Cannot find module '../../storage/models/account'
Require stack:
- D:\code\mail\src\cli\commands\delete.js
```

#### 复现步骤
1. 运行任何删除命令
2. 观察到模块未找到错误

#### 影响范围
- ❌ `mail-cli delete <id>` - 无法运行
- ❌ `mail-cli delete <id> --permanent` - 无法运行
- ✅ 其他命令 - 不受影响

#### 受影响文件
- `src/cli/commands/delete.js:3` - 错误的require语句
- `src/cli/commands/delete.js:36, 60` - 使用accountModel的地方

#### 根本原因
代码中尝试使用 `accountModel.findByEmail()` 来获取IMAP配置，但：
1. `src/storage/models/account.js` 文件不存在
2. 应该使用 `config.load()` 来获取IMAP配置（与其他命令保持一致）

#### 修复方案

**修改前**:
```javascript
const accountModel = require('../../storage/models/account');
// ...
const account = accountModel.findByEmail(email.from);
if (account) {
  const imapClient = new IMAPClient({
    user: account.email,
    password: account.imap_password,
    host: account.imap_host,
    port: account.imap_port,
    secure: account.imap_secure === 1
  });
}
```

**修改后**:
```javascript
const config = require('../../config');
// ...
const cfg = config.load();
if (cfg.imap.host && cfg.imap.user && cfg.imap.password) {
  const imapClient = new IMAPClient({
    user: cfg.imap.user,
    password: cfg.imap.password,
    host: cfg.imap.host,
    port: cfg.imap.port,
    secure: cfg.imap.secure
  });
}
```

#### 修复验证
- ✅ `mail-cli delete <id>` - 正常运行
- ✅ `mail-cli delete <id> --permanent` - 正常运行
- ✅ IMAP连接正常
- ✅ 邮件删除功能正常

---

## 阻塞性问题 (Blocker)

暂无

---

## 一般问题 (Major)

暂无

---

## 轻微问题 (Minor)

暂无

---

## 潜在改进建议

虽然不是 Bug，但以下是一些潜在的改进建议：

### 建议 #1: 添加依赖检查脚本
**描述**: 添加一个脚本来检查所有代码中使用的依赖是否都在 package.json 中声明。

**实现方案**:
```javascript
// scripts/check-dependencies.js
const fs = require('fs');
const path = require('path');

// 扫描所有 require() 语句
// 检查是否在 package.json 中声明
```

### 建议 #2: 添加单元测试
**描述**: 为所有新增功能添加单元测试，避免类似问题。

**实现方案**:
- 使用 Jest 框架
- 覆盖所有 CLI 命令
- 覆盖所有核心模块

### 建议 #3: 添加 CI/CD 流程
**描述**: 添加持续集成流程，自动运行测试和依赖检查。

**实现方案**:
- 使用 GitHub Actions
- 自动运行测试
- 自动检查依赖

### 建议 #4: 改进错误提示
**描述**: 当缺少依赖时，提供更友好的错误提示和修复建议。

**实现方案**:
```javascript
try {
  const Table = require('cli-table3');
} catch (error) {
  console.error('Missing dependency: cli-table3');
  console.error('Please run: npm install cli-table3');
  process.exit(1);
}
```

---

## Bug 修复优先级

### 立即修复 (P0)
1. ✅ Bug #1: 缺少依赖包 cli-table3

### 短期修复 (P1)
暂无

### 长期修复 (P2)
暂无

---

## 测试建议

### 回归测试清单
修复 Bug #1 后，需要执行以下回归测试：

#### 草稿功能测试
- [ ] `mail-cli draft save` - 保存草稿
- [ ] `mail-cli draft list` - 列出草稿 (重点测试)
- [ ] `mail-cli draft edit --id 1` - 编辑草稿
- [ ] `mail-cli draft send --id 1` - 发送草稿
- [ ] `mail-cli draft delete --id 1` - 删除草稿

#### 垃圾箱功能测试
- [ ] `mail-cli delete 1` - 删除邮件
- [ ] `mail-cli trash list` - 列出垃圾箱 (重点测试)
- [ ] `mail-cli trash restore 1` - 恢复邮件
- [ ] `mail-cli trash empty` - 清空垃圾箱

#### 表格显示测试
- [ ] 验证表格格式正确
- [ ] 验证表格内容完整
- [ ] 验证表格对齐正确
- [ ] 验证中文字符显示正常

---

## 质量保证措施

为避免类似问题再次发生，建议采取以下措施：

### 1. 代码审查
- 所有代码提交前进行 Code Review
- 检查新增依赖是否已声明
- 检查导入语句是否正确

### 2. 自动化测试
- 添加单元测试覆盖
- 添加集成测试
- 添加端到端测试

### 3. 依赖管理
- 使用 `npm-check` 检查依赖
- 定期更新依赖版本
- 锁定依赖版本 (package-lock.json)

### 4. 持续集成
- 配置 CI/CD 流程
- 自动运行测试
- 自动检查代码质量

### 5. 文档完善
- 更新开发文档
- 添加依赖说明
- 添加故障排查指南

---

## 附录

### A. 依赖检查命令
```bash
# 检查未使用的依赖
npm prune

# 检查过期的依赖
npm outdated

# 检查安全漏洞
npm audit

# 检查依赖树
npm list
```

### B. 测试命令
```bash
# 运行所有测试
npm test

# 运行测试覆盖率
npm run test:coverage

# 运行特定测试
npm test -- draft.test.js
```

### C. 相关文档
- [P0功能测试报告](./P0功能测试报告.md)
- [用户使用手册](./用户使用手册.md)
- [技术设计文档](./P0功能技术设计.md)

---

**文档版本**: v1.0
**最后更新**: 2026-02-08
**维护人**: QA Team

---

**注意**: 本文档会随着 Bug 的修复和新问题的发现持续更新。
